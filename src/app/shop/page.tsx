  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#0d1f33] text-[#eef3fb]">
        ...
        <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
          <div className="grid gap-8 lg:grid-cols-[3fr_1fr]">
                          ...
                          <div className="flex flex-col flex-grow p-6">
                            <div className="space-y-3">
                              ...
                              {product.colors.length > 0 && (
                                <div>
                                  ...
                                  <select
                                    ...
                                    className="w-full rounded-lg bg-[#0d1f33] px-3 py-2 text-sm text-[#eef3fb] border border-[#3f5a80] focus:outline-none focus:ring-2 focus:ring-[#faa153]"
                                  >
                              ...
                              <div className="grid gap-3">
                                <div className="flex items-center justify-between text-sm text-[#99a7c0]">
                                  <span>In cart</span>
            ...
            <aside className="hidden lg:block">
              <div className="sticky top-28 space-y-6 rounded-[2rem] border border-[#3f5a80] bg-[#0d1f33] p-6 shadow-sm">
                <div className="space-y-3">
                  <div>
                    <p className="text-sm font-semibold uppercase tracking-[0.24em] text-[#faa153]">Cart Summary</p>
                    <p className="text-xs text-[#99a7c0]">{cartCount} item{cartCount === 1 ? '' : 's'} selected</p>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-[#99a7c0]">Estimated total</span>
                    <span className="text-xl font-semibold text-[#eef3fb]">Rs {cartTotal}</span>
                  </div>
                </div>

                {cartItems.length === 0 ? (
                  <div className="rounded-3xl border border-dashed border-[#3f5a80] bg-[#162c47] p-6 text-center text-sm text-[#99a7c0]">
                    <p>Your cart is empty.</p>
                    <p className="mt-2">Add items from the collection to review them here.</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {cartItems.map((item) => (
                      <div key={`${item.id}-${item.selectedColor ?? 'default'}`} className="rounded-3xl border border-[#3f5a80] bg-[#162c47] p-4">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            <p className="text-sm font-semibold text-[#eef3fb]">{item.name}</p>
                            {item.selectedColor ? (
                              <p className="text-xs text-[#99a7c0]">Color: {item.selectedColor}</p>
                            ) : null}
                            <p className="text-xs text-[#99a7c0] mt-2">Available {item.quantity}</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <button
                              type="button"
                              onClick={() => updateQuantity(item.id, item.selectedQuantity - 1, item.selectedColor)}
                              className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-[#0d1f33] text-lg font-semibold text-[#faa153] transition hover:bg-[#1a2d42] disabled:cursor-not-allowed disabled:opacity-40"
                              disabled={item.selectedQuantity <= 1}
                            >
                              −
                            </button>
                            <span className="min-w-[2rem] text-center text-sm font-semibold text-[#eef3fb]">
                              {item.selectedQuantity}
                            </span>
                            <button
                              type="button"
                              onClick={() => updateQuantity(item.id, item.selectedQuantity + 1, item.selectedColor)}
                              className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-[#0d1f33] text-lg font-semibold text-[#faa153] transition hover:bg-[#1a2d42]"
                        ...
                        <div className="mt-4 flex items-center justify-between text-xs text-[#99a7c0]">
                ...
                <button
                  ...
                  className="mt-4 w-full rounded-full bg-[#faa153] px-4 py-3 text-sm font-semibold text-[#0d1f33] transition hover:bg-[#e69d6d] disabled:cursor-not-allowed disabled:opacity-50"
                >
      ...
      <button
        ...
        className="lg:hidden fixed bottom-6 right-6 z-40 inline-flex items-center justify-center rounded-full bg-[#faa153] px-5 py-3 text-sm font-semibold text-[#0d1f33] shadow-lg shadow-[#0d1f33]/40 transition hover:bg-[#e69d6d]"
      >
        View Cart ({cartCount})
      </button>

      {isCartOpen ? (
        <div className="fixed inset-0 z-50 flex bg-[#0d1f33]/60 p-4 lg:hidden">
          <div className="absolute inset-0" onClick={() => setIsCartOpen(false)} />
          <div className="relative ml-auto flex h-full w-full max-w-sm flex-col overflow-y-auto rounded-[2rem] border border-[#3f5a80] bg-[#0d1f33] p-6 shadow-2xl">
            <div className="mb-6 flex items-center justify-between gap-4">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.24em] text-[#faa153]">Your Cart</p>
                <p className="text-xs text-[#99a7c0]">{cartCount} item{cartCount === 1 ? '' : 's'}</p>
              </div>
              <button
                type="button"
                onClick={() => setIsCartOpen(false)}
                className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-[#162c47] text-[#eef3fb] transition hover:bg-[#1a2d42]"
            ...
            {cartItems.length === 0 ? (
              <div className="rounded-3xl border border-dashed border-[#3f5a80] bg-[#162c47] p-6 text-sm text-[#99a7c0]">
                <p>Your cart is empty.</p>
              ...
              <div className="space-y-5">
                ...
                <div className="rounded-3xl border border-[#3f5a80] bg-[#162c47] p-4">
                  <div className="flex items-center justify-between text-sm text-[#99a7c0]">
                    <span>Total</span>
                    <span className="font-semibold text-[#eef3fb]">Rs {cartTotal}</span>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="mt-4 w-full rounded-full bg-[#faa153] px-4 py-3 text-sm font-semibold text-[#0d1f33] transition hover:bg-[#e69d6d]"
                  >
                    Proceed to Generate Receipt
                  </button>
                </div>

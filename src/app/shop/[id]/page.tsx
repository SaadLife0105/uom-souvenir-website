      ...
      <main className="flex-1 pb-20 pt-28" style={{ backgroundColor: floralWhiteHex }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <nav aria-label="Breadcrumb" className="mt-4 flex flex-wrap items-center gap-1.5 text-sm text-[#5b6b86]">
            ...
            <ChevronRight className="h-3.5 w-3.5" aria-hidden />
            <span style={{ color: camelHex }}>{product.name}</span>
          </nav>
          ...
          <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_1fr_300px] lg:items-start">
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="grid grid-cols-1 gap-4 rounded-3xl p-5 shadow-md sm:grid-cols-3" style={{ backgroundColor: whiteSmokeHex }}>
                {TRUST_BADGES.map(({ icon: Icon, title, description }) => (
                  <div key={title} className="flex items-start gap-3">
                    <Icon className="mt-0.5 h-5 w-5 shrink-0" style={{ color: camelHex }} />
                    <div>
                      <p className="text-sm font-semibold" style={{ color: camelHex }}>{title}</p>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div>
                <h1 className="text-3xl font-bold" style={{ color: camelHex }}>
                  {product.name}
                </h1>
              ...
              <p className="text-2xl font-bold" style={{ color: camelHex }}>
              ...
              <div className="flex flex-col divide-y" style={{ borderColor: subtleBorder }}>
                {INFO_SECTIONS.map(({ title, body }) => (
                  <details key={title} className="group py-2" style={{ borderColor: subtleBorder }}>
                    <summary className="flex cursor-pointer list-none items-center justify-between py-2 text-sm font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2" style={{ color: camelHex, outlineColor: camelHex }}>
                      {title}

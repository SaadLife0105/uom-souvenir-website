...
const featureIcons: Record<FeatureCardProps["icon"], ReactElement> = {
  shield: (
    <svg viewBox="0 0 24 24" className="h-6 w-6 text-[#faa153]" fill="none" stroke="currentColor" strokeWidth="1.5">
...
export default function FeatureCard({ title, description, icon }: FeatureCardProps) {
  return (
    <div className="rounded-[2rem] border border-[#3f5a80] bg-[#0d1f33] p-6 text-[#eef3fb] shadow-sm transition hover:-translate-y-1 hover:shadow-[0_24px_80px_-50px_rgba(0,0,0,0.28)]">
      <div className="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-3xl bg-[#faa153] text-[#0d1f33]">
        {featureIcons[icon]}
      </div>
      <h3 className="text-lg font-semibold">{title}</h3>
      <p className="mt-3 text-sm leading-7 text-[#99a7c0]">{description}</p>
    </div>
  );
}
